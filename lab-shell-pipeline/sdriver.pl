...
while (<INFILE>) {
    ...
    # Other input
        ...
        # Tempfile
        if (@tmpfilenamearr = ($line =~ /(TEMPFILE\d+)/g)) {
	    foreach $tmpfileplaceholder (@tmpfilenamearr) {
                if (!defined($tmpfilenames{$tmpfileplaceholder})) {
                    $tmpfileplaceholder =~ /TEMPFILE(\d+)/;
                    ($tmpfh, $tmpfilename) = tempfile("tshtmp-$1-XXXXXX");
	            printf $tmpfh "$tmpfilename\n";
                    close($tmpfh);
	            if ($verbose) {
	                print "$0: Created $tmpfileplaceholder as $tmpfilename\n";
	            }
                    $tmpfilenames{$tmpfileplaceholder} = $tmpfilename;
	        }
                $tmpfilename = $tmpfilenames{$tmpfileplaceholder};
	        $line =~ s/$tmpfileplaceholder/$tmpfilename/g;
            }
        }
	if ($verbose) {
...
foreach $num (keys %tmpfilenames) {
    if ($verbose) {
        print "$0: Unlinking $tmpfilenames{$num}\n";
    }
    unlink $tmpfilenames{$num};
}
